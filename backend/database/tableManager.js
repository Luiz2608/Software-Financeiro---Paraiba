...
async function maybeSeed(client) {
  try {
    ...
    const topup = String(process.env.AUTO_SEED_TOPUP || 'true').toLowerCase() === 'true';
    if (!topup) {
      console.log(`ℹ️ Auto-seed top-up desativado (pessoas=${p}, classificacao=${c}, movimentos=${m})`);
      return;
    }

    const targetP = Number(process.env.AUTO_SEED_TARGET_PESSOAS || 200);
    const targetC = Number(process.env.AUTO_SEED_TARGET_CLASSIFICACAO || 50);
    const targetM = Number(process.env.AUTO_SEED_TARGET_MOVIMENTOS || 200);

    const addP = Math.max(0, targetP - p);
    const addC = Math.max(0, targetC - c);
    const addM = Math.max(0, targetM - m);

    if (addP > 0) {
      const forn = Math.floor(addP / 2);
      const fat = addP - forn;
      if (forn > 0) {
        console.log(`➕ Inserindo ${forn} fornecedores (top-up)`);
        await client.query(
          `INSERT INTO pessoas (tipo, razao_social, nome_fantasia, cnpj_cpf, endereco, telefone, email, ativo, data_cadastro)
           SELECT 'FORNECEDOR',
                  'Fornecedor ' || gs,
                  'Fornecedor ' || gs,
                  '99' || LPAD((floor(random()*10000000000000))::text, 14, '0'),
                  'Rua ' || gs || ', Centro',
                  '62-' || LPAD((floor(random()*100000000))::text, 8, '0'),
      ...
      if (fat > 0) {
        console.log(`➕ Inserindo ${fat} faturados (top-up)`);
        await client.query(
          `INSERT INTO pessoas (tipo, razao_social, nome_fantasia, cnpj_cpf, endereco, telefone, email, ativo, data_cadastro)
           SELECT 'FATURADO',
                  'Cliente ' || gs,
                  'Cliente ' || gs,
                  '88' || LPAD((floor(random()*10000000000000))::text, 14, '0'),
                  'Av ' || gs || ', Bairro',
                  '64-' || LPAD((floor(random()*100000000))::text, 8, '0'),
    ...
    if (addC > 0) {
      console.log(`➕ Inserindo ${addC} classificações (top-up)`);
      await client.query(
    ...
    if (addM > 0) {
      console.log(`➕ Inserindo ${addM} movimentos/parcelas/vínculos (top-up)`);
      await client.query(
        `WITH new_movs AS (
           ...
           SELECT CASE WHEN gs % 2 = 0 THEN 'APAGAR' ELSE 'ARECEBER' END,
                  (SELECT id FROM pessoas ORDER BY random() LIMIT 1),
                  LPAD(gs::text, 9, '0'),
      ...
      await client.query(
        `WITH nm AS (
           SELECT id, valor_total FROM movimentocontas ORDER BY id DESC LIMIT $1
         )
